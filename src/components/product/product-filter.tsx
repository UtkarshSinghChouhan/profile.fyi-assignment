    ...
    return (
        <>
            <aside className='sticky hidden xl:flex flex-col top-[71px] h-[calc(100vh-71px)] w-[210px] overflow-auto scroll-hidden mr-6 py-[18px]'>
                <h3 className='text-black select-none font-semibold px-4 underline underline-offset-2 mb-2'>CATEGORIES</h3>
                ...
                {categoriesData?.map((obj: any, idx: number) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn('bg-white text-left hover:bg-pf-light-gray py-2 px-4', isActive && 'bg-pf-light-gray')}
                        >
                            {obj.name}
                        </button>
                    )
                ...
                <h3 className='text-black select-none font-semibold px-4 underline underline-offset-2 mt-4 mb-2'>SORT BY</h3>
                {SORT_BY.map((obj, idx) => {
                    ...
                    return (
                        <button
                            key={idx}
                            onClick={() => {
                                router.push('?' + createQueryString({'sort_by': obj.sortBy, 'order': obj.order}))
                            }}
                            className={cn('bg-white text-left hover:bg-pf-light-gray py-2 px-4', isActive && 'bg-pf-light-gray')}
                        >
                            {obj.label}
